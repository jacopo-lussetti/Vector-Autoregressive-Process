...
iter<- 100 #number of iteration for monte carlo
...
#we create a list to store coef estimates and their relative betas
...
for(gamma_idx in seq_along(gammas)){
  ...
  #compute beta
    ...
    Coef_estimate[[gamma_idx]][[n_idx]] <- list(
      beta_hat_mat = beta_hat_mat,   # K x iter
      beta_star_mat = beta_star_mat, # K x iter
      lasso_error_iter = lasso_error_mc  # length iter
    )
